...
export const emptyCardElem = {
  getCard: () => screen.getByTestId('card-empty'),
  getTitleEl: () => within(emptyCardElem.getCard()).queryByPlaceholderText('Tytuł...'),
  getListItemTextarea: () =>
    within(emptyCardElem.getCard()).getAllByPlaceholderText('Utwórz listę...') as unknown as HTMLTextAreaElement[],
...
export const editCardElem = {
  ...
  getListItemTextarea: (id: string = '0') =>
    within(editCardElem.getEditCard(id)).getAllByPlaceholderText('Utwórz listę...') as unknown as HTMLTextAreaElement[],
  getListItems: (id: string = '0') => within(editCardElem.getEditCard(id)).getAllByRole('listitem'),
  getAddElButton: (id: string = '0') =>
    within(editCardElem.getEditCard(id)).getByRole('button', { name: '+ Element listy' }),
  getDeleteButton: (index: number = 0) => within(editCardElem.getListItems()[index]).getByLabelText('Delete'),
  getCheckbox: (id: string = '0', checkboxId: string) =>
    within(editCardElem.getEditCard(id)).getByTestId(`list-item-${checkboxId}-checkbox`),
  queryCheckbox: (id: string = '0', checkboxId: string) =>
    within(editCardElem.getEditCard(id)).queryByTestId(`list-item-${checkboxId}-checkbox`),
  getEditIndicator: (id: string = '0') => screen.queryByTestId(`card-${id}-edit-indicator`),
  getDoneElemExpandButton: (id: string = '0') =>
    within(editCardElem.getEditCard(id)).queryByText('ukończonych elementów', { exact: false }),
