const EditIndicator = ({id, isEdit}:{id: string, isEdit: boolean}) => {
  return (
    <div data-testid={`card-${id}-edit-indicator`} className={`transition-opacity duration-300 ${isEdit ? 'opacity-100' : 'opacity-0'}`} aria-hidden={`${isEdit ? "false" : "true"}`}>
